...
class PDFService:
    ...
    def extract_tables_from_pdf(pdf_path: str) -> List[Dict]:
        """
        Extract tables from PDF with improved detection
        Returns list of tables as dictionaries
        """
        tables = []
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    for table_num, table in enumerate(page_tables, 1):
                        if table and len(table) > 0:
                            ...
                            cleaned_table = PDFService._clean_table(table)
                            
                            if cleaned_table and len(cleaned_table) > 1:
                                headers = cleaned_table[0] if cleaned_table[0] else []
                                ...
                                tables.append(table_dict)
                                print(f"✓ Extracted table {table_num} from page {page_num} ({len(rows)} rows)")
