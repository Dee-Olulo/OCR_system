    ...
    try:
        ...
        if file_ext == ".pdf":
            ...
            if is_scanned:
                ...
                print(f"PDF is scanned, converting to images...")
                image_paths = pdf_service.convert_pdf_to_images(file_path)
                
                for img_path in image_paths:
                    result = ocr_service.extract_text(img_path, engine)
                    if result["success"]:
            ...
            # Regular image - use OCR
            result = ocr_service.extract_text(file_path, engine)
            if result["success"]:
                extracted_text = result["text"]
            else:
        ...
        if classify_document and extracted_text:
            doc_type, classification_confidence = document_classifier.classify_document(extracted_text)
        ...
        # CRITICAL: Save JSON to both file path AND database
        if generate_json and json_output_data:
            update_data["json_output_path"] = json_file_path
            update_data["json_output"] = json_output_data  # ← SAVE TO DATABASE
        ...
        return {
            ...
            "json_output": json_output_data if generate_json else None,  # Return in response
            "message": "Document processed successfully with Phase 2 features"
        }
...
async def get_advanced_result(
    ...
    """
    Get advanced processing results including entities, tables, and JSON output
    
    CRITICAL: This endpoint now returns json_output directly from the database
    """
    db = get_database()
    ...
    if json_data:
        print(f"✓ Returning json_output for document {document_id}")
    else:
        print(f"✗ No json_output available for document {document_id}")
    ...
    response = {
        ...
        "json_output": json_data,  # ← CRITICAL: Include JSON output in response
